...
import {
  ...
  HiCheck, HiArrowLeft, HiArrowRight,
} from "react-icons/hi";
...
const emptyForm: AddressForm = {
  title: "", receiver_name: "", receiver_mobile: "",
  province: "", city: "", address: "", postal_code: "", is_default: false,
};
...
function AddressFormModal({
  open, onClose, onSave, editAddress,
}: {
  open: boolean;
  onClose: () => void;
  ...
  const inp = "w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition";
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition">
            ✕
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input placeholder="مثلاً: خانه، محل کار" value={form.title}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input required value={form.receiver_name}
            ...
            <div>
              ...
              <input required dir="ltr" value={form.receiver_mobile}
                onChange={(e) => setForm({ ...form, receiver_mobile: e.target.value })}
                maxLength={11} placeholder="09xxxxxxxxx" className={inp} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input required value={form.province}
          ...
          <div>
            ...
            <input required dir="ltr" value={form.postal_code}
              onChange={(e) => setForm({ ...form, postal_code: e.target.value })}
              maxLength={10} placeholder="1234567890" className={inp} />
          </div>
...
export default function CheckoutAddressPage() {
  ...
  const handleContinue = () => {
    ...
    // آدرس انتخابی رو توی sessionStorage ذخیره کن
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">انتخاب آدرس تحویل</h1>
        ...
        <div className="flex items-center gap-2 mb-8">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold">۱</div>
            <span className="text-sm font-medium text-blue-600">آدرس تحویل</span>
          </div>
          <div className="flex-1 h-0.5 bg-gray-200 mx-2" />
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gray-200 text-gray-400 rounded-full flex items-center justify-center text-sm font-bold">۲</div>
            ...
            {addresses.length === 0 ? (
              ...
              addresses.map((addr) => (
                <div key={addr.id}
                  ...
                  {/* تیک انتخاب */}
                  <div className={`absolute top-4 left-4 w-6 h-6 rounded-full border-2 flex items-center justify-center transition ${
                    selectedId === addr.id
                      ? "bg-blue-600 border-blue-600"
                      : "border-gray-300"
                  }`}>
                    {selectedId === addr.id && <HiCheck className="w-3.5 h-3.5 text-white" />}
                  </div>

                  <div className="pr-1 pl-10">
                    {/* عنوان */}
                    <div className="flex items-center gap-2 mb-2">
                      <HiLocationMarker className={`w-4 h-4 flex-shrink-0 ${selectedId === addr.id ? "text-blue-500" : "text-gray-400"}`} />
                    ...
                    {/* آدرس */}
                    <p className="text-sm text-gray-600 mb-2 leading-relaxed">{addr.address}</p>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                    <button
                      onClick={(e) => { e.stopPropagation(); setEditingAddress(addr); setModalOpen(true); }}
