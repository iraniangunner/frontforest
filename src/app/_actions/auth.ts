...
export async function verifyOtpAction(prevState: any, formData: FormData) {
  ...
  try {
    ...
    // ── ۳ نشست فعال — نیاز به مدیریت نشست ──
    if (res.status === 409 && data.require_session_management) {
      return {
        isSuccess: false,
        error: "",
...
export async function revokeSessionBeforeLoginAction(
  ...
  try {
    ...
    if (!res.ok) {
      return { success: false, error: data.message || "خطا در حذف نشست" };
    }
...
export async function loginAfterRevokeAction(sessionToken: string) {
  ...
  try {
    ...
    if (!res.ok) {
      return { success: false, error: data.message || "خطا در ورود" };
    }
