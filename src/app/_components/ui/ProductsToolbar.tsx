...
// app/products/_components/ProductsToolbar.tsx
...
interface Category { id: number; name: string; slug: string; children?: any[]; }
...
export default function ProductsToolbar({ total, categories, priceRange }: Props) {
  const sp                    = useSearchParams();
  const { push, clearAll, isPending } = useFilterPush();
  ...
  const chips: { key: string; value?: string; label: string }[] = [
    ...sp.getAll("categories[]").map(v => ({ key: "categories[]", value: v, label: getCatName(v) })),
    sp.get("on_sale")  === "1" ? { key: "on_sale",  label: "تخفیف‌دار" }     : null,
    sp.get("in_stock") === "1" ? { key: "in_stock", label: "موجود در انبار" } : null,
    sp.get("min_price") || sp.get("max_price") ? {
      key: "price",
      label: `${Number(sp.get("min_price")||priceRange.min).toLocaleString("fa-IR")} — ${
        Number(sp.get("max_price")||priceRange.max).toLocaleString("fa-IR")} ت`,
    } : null,
    sp.get("min_rating") ? { key: "min_rating", label: `${sp.get("min_rating")}★+` } : null,
    sp.get("q")          ? { key: "q",           label: `"${sp.get("q")}"` }          : null,
  ].filter(Boolean) as { key: string; value?: string; label: string }[];

  const removeChip = (key: string, value?: string) => {
    if (key === "categories[]") {
      const rest = sp.getAll("categories[]").filter(v => v !== value);
      push({ "categories[]": rest });
    } else if (key === "price") {
  ...
  return (
    <>
      <div className={`bg-white rounded-xl border px-4 py-3 space-y-2.5 transition-colors ${
        isPending ? "border-teal-200 bg-teal-50/30" : "border-gray-200"
      }`}>
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm text-gray-600 flex items-center gap-2">
            <span className="font-semibold text-gray-900">{total.toLocaleString("fa-IR")}</span>
            محصول
            {/* نشانگر loading */}
            {isPending && (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select value={perPage} onChange={e => set("per_page", e.target.value)}
              className="text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-teal-500 bg-white">
              {PER_PAGE.map(n => <option key={n} value={n}>{n} عدد</option>)}
            </select>

            <div className="flex border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={() => set("view", "grid")}
        ...
        {chips.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-2 border-t border-gray-100">
            {chips.map((c, i) => (
              <button key={i} onClick={() => removeChip(c.key, c.value)}
      ...
      {drawer && (
        <div className="fixed inset-0 z-50 flex lg:hidden" dir="rtl">
          ...
          <div className="relative mr-auto w-72 max-w-[85vw] h-full bg-white shadow-2xl flex flex-col">
            <ProductsFilter categories={categories} priceRange={priceRange} isMobile onClose={() => setDrawer(false)} />
          </div>
        </div>
      )}
    </>
