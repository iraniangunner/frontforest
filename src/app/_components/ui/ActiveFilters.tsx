...
export default function ActiveFilters({ categories, tags }: ActiveFiltersProps) {
  ...
  const handleRemove = (key: string, value?: string) => {
    ...
    } else if (value && key.endsWith("[]") || ["categories", "frameworks", "stylings", "features"].includes(key)) {
      const paramKey = key.endsWith("[]") ? key : `${key}[]`;
      const values = params.getAll(paramKey).filter((v) => v !== value);
  ...
  if (filters.min_price || filters.max_price) {
    const min = filters.min_price
      ? parseInt(filters.min_price).toLocaleString("fa-IR")
      : "۰";
    const max = filters.max_price
      ? parseInt(filters.max_price).toLocaleString("fa-IR")
      : "∞";
    activeFilters.push({
      key: "price",
      label: `${min} - ${max} تومان`,
        ...
        {activeFilters.map((filter, index) => {
          ...
          return (
            <button
              ...
              className={`group inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 hover:scale-105 active:scale-95 ${
                filter.color ? "" : `${style.bg} ${style.text}`
              }`}
              style={
                filter.color
                  ? {
