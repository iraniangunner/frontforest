          ...
          <div className="space-y-4">
            {addresses.length === 0 ? (
              ...
              addresses.map((addr) => (
                ...
                >
                  <div
                    className={`absolute top-4 left-4 w-6 h-6 rounded-full border-2 flex items-center justify-center transition ${selectedId === addr.id ? "bg-blue-600 border-blue-600" : "border-gray-300"}`}
                  >
                    {selectedId === addr.id && (
                      <HiCheck className="w-3.5 h-3.5 text-white" />
                    )}
